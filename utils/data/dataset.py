...
def build_samples_from_questions_file(
    ...
    # skip file if not the requested level
    if request_level is not None and level != request_level:
        return []
...
def build_all_samples(
    ...
    """
    Build samples from multiple question files.
    """
...
class DsAdapterSpatial457PerLevel:
    def __init__(
        self,
        request_split=SPLIT_NAME_TEST,
        request_level="L1_single",
    ):
        ...
        self.samples = build_all_samples(
            questions_dir,
            self.images_by_name,
            request_level=request_level,
        )
